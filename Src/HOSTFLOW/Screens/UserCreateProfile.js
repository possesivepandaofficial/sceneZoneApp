...
// console.log(userData, "User Data from Redux");
...
//             address: address,
//             profileImageUrl:
//               response.data.data.user.profileImageUrl ||
...
//     } catch (error) {
//       console.error(
//         `Error ${isEditing ? "updating" : "creating"} profile:`,
//         error
//       );
//       Alert.alert(
//         "Error",
...
const UserCreateProfileScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (userData?.fullName && userData?.email && userData.email !== "") {
      setIsEditing(true);
    }
    setFullName(initialFullName);
    setPhoneNumber(initialPhoneNumber);
  ...
  const handleImageResponse = (response) => {
    ...
    if (selectedImage && validateImage(selectedImage)) {
      console.log("Selected profileImage:", selectedImage);
      setProfileImage(selectedImage);
    }
  };
    ...
    try {
      ...
      console.log("Sending FormData:", {
        ...
        profileImage: profileImage
          ? { uri: profileImage.uri, type: profileImage.type, name: profileImage.fileName }
          : null,
      });
      console.log("Token:", token);
      console.log("Endpoint:", isEditing ? "/user/update-profile" : "/user/create-profile");
      ...
      if (response.data.success) {
        dispatch(
          loginUser({
            id: response.data.data.user._id || userData?.id,
            name: fullName,
            fullName: fullName,
            email: email,
            phone: phoneNumber,
        ...
        Alert.alert(
          "Success",
          response.data.message || `Profile ${isEditing ? "updated" : "created"} successfully!`
        );
        navigation.reset({
          index: 0,
        ...
        throw new Error(response.data.message || `Failed to ${isEditing ? "update" : "create"} profile`);
      }
    } catch (error) {
        ...
        >
          ...
          >
            <View style={styles.profileImageContainer}>
              <Image
                source={
                  profileImage ? { uri: profileImage.uri } : require("../assets/Images/frame1.png")
                }
                style={styles.profileImage}
              />

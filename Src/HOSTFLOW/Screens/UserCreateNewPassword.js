...
const UserCreateNewpasswordScreen = ({ navigation, route }) => {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const response = await api.post("/user/set-newpassword", {
        email,
        password,
      });
  ...
  <View style={styles.modalBackground}>
    <View style={styles.modalContent}>
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          setShowSuccessModal(false);
          navigation.replace("UserHome"); // or navigate()
        }}
      >
        <LinearGradient
          colors={["#B15CDE", "#7952FC"]}
